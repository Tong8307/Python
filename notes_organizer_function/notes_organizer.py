...
class _CropCanvas(QWidget):
    ...
    def paintEvent(self, e):
        """Draw image, dark overlays, selection, and handles."""
        p = QPainter(self)
...
class NoteTabWidget(QWidget):
    ...
    def __init__(self, note_id, title="", content="", overlay=None):
        ...
        def _build_eraser_menu():
            ...
            act_n.setChecked(not is_lasso) if False else act_n.setChecked(not is_lasso)  # keep exact behavior
...
class NoteOrganizerWidget(QWidget):
    ...
    def _gc_deleted_tabs(self):
        ...
        for i in reversed(range(self.tabs.count())):
            ...
            if isinstance(w, NoteTabWidget):
                ...
                if nid is None or not db.get_note(nid, user_id=self.user_id):
                    self.tabs.removeTab(i)
                    removed_any = True
        if removed_any:
            if self.tabs.count() == 0:
    ...
    def _open_by_id(self, nid: int):
        ...
        tab = NoteTabWidget(nid, row.get("title","Untitled"), row.get("content",""), overlay=overlay)
    ...
    def _save_active(self, show_popup: bool=False):
        ...
        if not db.get_note(w.note_id, user_id=self.user_id):
