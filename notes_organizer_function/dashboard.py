...
from PyQt5.QtGui import QIcon, QPixmap, QPdfWriter, QPainter
...
class DashboardWidget(QWidget):
    def __init__(self, user_id, on_add_note_clicked=None, on_back_clicked=None):
        super().__init__()
        if not user_id:
            raise ValueError("DashboardWidget requires a user_id")
    ...
    def _setup_top(self):
        ...
        self.back_button = QPushButton("Back", objectName="backBtn")
    ...
    def _on_back(self):
        """Call the provided back callback (if any)."""
        if callable(self.on_back_clicked):
            try:
                self.on_back_clicked()
                return
            except Exception as e:
                # swallow and continue to default behavior
                print("on_back_clicked raised:", e)
        # default fallback: hide the widget (host app can handle showing previous screen)
        self.hide()
    ...
    def _refresh_folders(self):
        ...
        conn = self._db(); cur = conn.cursor()
        # Only get folders for the current user
        cur.execute("""
        ...
        def add_branch(parent_id, level=0):
            for fid, name in sorted(tree.get(parent_id, []), key=lambda t: t[1].lower()):
                ...
                if fid in self._expanded_folders:
                    ...
                    # inline notes inside the expanded folder branch - only for current user
                    conn2 = self._db(); cur2 = conn2.cursor()
                    cur2.execute("SELECT id, title FROM notes WHERE folder_id = ? AND user_id = ? ORDER BY LOWER(title)", 
                                (fid, self.user_id))
    ...
    def _fetch_notes(self):
        ...
        sql = "SELECT id, title, COALESCE(updated_at, created_at) AS modified_at FROM notes WHERE user_id = ?"
        args = [self.user_id]
        ...
        if self.current_folder_id == -1:
            ...
            sql += " AND folder_id = ?"
            args.append(self.current_folder_id)
        ...
        # Add ordering
        if self.sort_mode == 0:
            sql += " ORDER BY modified_at DESC, LOWER(title)"
        else:
    ...
    # In the _child_folders method, add user_id filter:
    def _child_folders(self):
        # children of current folder (or roots) - only for current user
    ...
    def _add_note_here(self, folder_id):
        ...
        user_id = self.user_id  # use widget's stored user id

        conn = self._db(); cur = conn.cursor()
        cur.execute("INSERT INTO notes(folder_id, title, content, user_id) VALUES(?,?,?,?)",
                    (target, "Untitled", "", user_id))
    ...
    def _import_note(self, folder_id):
        ...
        # Get current user ID from parent (main window)
        user_id = None
        if hasattr(self, 'parent') and hasattr(self.parent(), 'user_id'):
            user_id = self.parent().user_id

        conn = self._db(); cur = conn.cursor()
        cur.execute("INSERT INTO notes(folder_id, title, content, user_id) VALUES (?,?,?,?)", 
                    (target, title, content, user_id))
        nid = cur.lastrowid; conn.commit(); conn.close()
    ...
    def _choose_folder_dialog(self):
        ...
        def walk(parent=None, level=0):
            for fid, name in sorted(tree.get(parent, []), key=lambda t: t[1].lower()):
                flat.append((fid, name, ("    " * level) + name))
                walk(fid, level+1)
        walk(None)
        if not flat:
            QMessageBox.information(self, "Move Note", "No folders available. Create a folder first.")
            return None
