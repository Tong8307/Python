# database/init_db.py
import os
import sqlite3
import hashlib
import secrets

# ---------- Helpers ----------
def hash_password(password, salt=None):
    """Hash password with per-user salt using SHA-256."""
    if salt is None:
        salt = secrets.token_hex(16)  # 16 bytes -> 32 hex chars
    hashed = hashlib.sha256((password + salt).encode()).hexdigest()
    return hashed, salt

# ---------- Connect (DB inside /database) ----------
DB_PATH = os.path.join(os.path.dirname(__file__), "student_app.db")
conn = sqlite3.connect(DB_PATH)
cursor = conn.cursor()
cursor.execute("PRAGMA foreign_keys = ON")

# ---------- Schema ----------
# 1) Users
cursor.execute("""
CREATE TABLE IF NOT EXISTS users (
    student_id TEXT PRIMARY KEY,
    name TEXT NOT NULL,
    password_hash TEXT NOT NULL,
    password_salt TEXT NOT NULL,
    profile_picture TEXT
)
""")

# 2) Locations
cursor.execute("""
CREATE TABLE IF NOT EXISTS locations (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL
)
""")

# 3) Rooms
cursor.execute("""
CREATE TABLE IF NOT EXISTS rooms (
    id TEXT PRIMARY KEY,
    location_id INTEGER NOT NULL,
    capacity INTEGER NOT NULL,
    name TEXT NOT NULL,
    feature_id TEXT,
    FOREIGN KEY (location_id) REFERENCES locations(id),
    FOREIGN KEY (feature_id)  REFERENCES features(id)
)
""")

# 4) Features
cursor.execute("""
CREATE TABLE IF NOT EXISTS features (
    id TEXT PRIMARY KEY,
    name TEXT NOT NULL
)
""")

# 5) Bookings
cursor.execute("""
CREATE TABLE IF NOT EXISTS bookings (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    room_id TEXT NOT NULL,
    date TEXT NOT NULL,
    start_time TEXT NOT NULL,
    end_time TEXT NOT NULL,
    status TEXT CHECK(status IN ('booked','cancelled','completed')) NOT NULL DEFAULT 'booked',
    created_by TEXT NOT NULL,
    FOREIGN KEY (room_id)    REFERENCES rooms(id),
    FOREIGN KEY (created_by) REFERENCES users(student_id)
)
""")

# 6) Booking_Students
cursor.execute("""
CREATE TABLE IF NOT EXISTS booking_students (
    booking_id INTEGER NOT NULL,
    student_id TEXT NOT NULL,
    student_name TEXT,
    PRIMARY KEY (booking_id, student_id),
    FOREIGN KEY (booking_id) REFERENCES bookings(id),
    FOREIGN KEY (student_id) REFERENCES users(student_id)
)
""")

# 7) Folders
cursor.execute("""
CREATE TABLE IF NOT EXISTS folders (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL,
    parent_id INTEGER,
    color TEXT DEFAULT '#FFFFFF',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (parent_id) REFERENCES folders(id)
)
""")

# 8) Notes
cursor.execute("""
CREATE TABLE IF NOT EXISTS notes (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    folder_id INTEGER NULL,
    title TEXT NOT NULL,
    content TEXT,
    cover_path TEXT,
    file_path TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (folder_id) REFERENCES folders(id)
)
""")

# Helpful indexes for notes
cursor.execute("CREATE INDEX IF NOT EXISTS idx_notes_title   ON notes(title)")
cursor.execute("CREATE INDEX IF NOT EXISTS idx_notes_updated ON notes(updated_at)")

# ---------- Seed data ----------
# Locations
cursor.executemany(
    "INSERT OR IGNORE INTO locations (id, name) VALUES (?, ?)",
    [
        (1, "Cyber Centre Discussion Room"),
        (2, "Library Discussion Room"),
        (3, "Arena Discussion Room"),
    ],
)

# Features
cursor.executemany(
    "INSERT OR IGNORE INTO features (id, name) VALUES (?, ?)",
    [
        ("F01", "Discussion Room (1PC)"),
        ("F02", "Discussion Room (2PCS)"),
        ("F03", "Discussion Room (2PCS)"),
        ("F04", "Discussion Room with Projector (2PCS)"),
        ("F05", "Discussion Room with Projector (2PCS) [HDMI]"),
    ],
)

# Rooms
cursor.executemany(
    "INSERT OR IGNORE INTO rooms (id, location_id, capacity, name, feature_id) VALUES (?, ?, ?, ?, ?)",
    [
        ("R101", 1,  6, "Room C204", "F01"),
        ("R102", 1,  8, "Room C280", "F02"),
        ("R201", 2,  4, "Room L222", "F03"),
        ("R202", 2, 10, "Room L888", "F04"),
        ("R301", 3,  5, "Room A012", "F05"),
        ("R302", 3, 12, "Room A125", "F05"),
        ("R303", 3,  5, "Room A012", "F02"),
        ("R304", 3, 10, "Room A124", "F01"),
        ("R305", 3,  2, "Room A028", "F03"),
        ("R306", 3,  3, "Room A129", "F04"),
        ("R307", 3,  6, "Room A038", "F02"),
        ("R308", 3,  5, "Room A175", "F03"),
    ],
)

# Users (hashed)
users_data = [
    ("24WMD0624", "Eun Eun Bond",  "pass123",    "user1.png"),
    ("24WMD0345", "Yu Yu Bond",    "abc456",     "user2.png"),
    ("24WMD0188", "Tong Tong Bond","123456",     "user3.png"),
    ("24WMD0199", "John Tan",      "johnpwd",    "user4.png"),
    ("24WMD0222", "Nur Aisyah",    "aisyah@123", "user5.png"),
]
for sid, name, pwd, pic in users_data:
    ph, ps = hash_password(pwd)
    cursor.execute(
        "INSERT OR IGNORE INTO users (student_id, name, password_hash, password_salt, profile_picture) VALUES (?, ?, ?, ?, ?)",
        (sid, name, ph, ps, pic),
    )

# ---------- Reset + seed bookings safely ----------
# Delete children first, then parents
cursor.execute("DELETE FROM booking_students")
cursor.execute("DELETE FROM bookings")

# Insert bookings one-by-one and capture real IDs
bookings_seed = [
    ("R101", "2025-08-01", "10:00", "12:00", "booked",    "24WMD0624"),
    ("R202", "2025-08-02", "14:00", "16:00", "booked",    "24WMD0345"),
    ("R201", "2025-08-03", "09:00", "11:00", "cancelled", "24WMD0188"),
    ("R301", "2025-08-04", "15:00", "17:00", "booked",    "24WMD0199"),
]
booking_ids = []
for row in bookings_seed:
    cursor.execute(
        "INSERT INTO bookings (room_id, date, start_time, end_time, status, created_by) VALUES (?, ?, ?, ?, ?, ?)",
        row,
    )
    booking_ids.append(cursor.lastrowid)

# Participants use indices into bookings_seed (0-based)
participants = [
    (0, "24WMD0624", "Eun Eun Bond"),
    (0, "24WMD0345", "Yu Yu Bond"),
    (1, "24WMD0345", "Yu Yu Bond"),
    (1, "24WMD0222", "Nur Aisyah"),
    (2, "24WMD0188", "Lim Mei Mei"),
    (3, "24WMD0199", "John Tan"),
    (3, "24WMD0624", "Eun Eun Bond"),
]
for idx, sid, sname in participants:
    cursor.execute(
        "INSERT INTO booking_students (booking_id, student_id, student_name) VALUES (?, ?, ?)",
        (booking_ids[idx], sid, sname),
    )

# ---------- Seed a welcome note if notes is empty ----------
cursor.execute("SELECT COUNT(*) FROM notes")
if cursor.fetchone()[0] == 0:
    cursor.execute(
        "INSERT INTO notes (folder_id, title, content) VALUES (?, ?, ?)",
        (
            None,
            "Welcome to Your Notes App",
            "👋 Welcome! Use the ➕ button to add notes. You can organize notes into folders anytime.",
        ),
    )

conn.commit()
print(f"Database initialized successfully at: {DB_PATH}")
conn.close()
